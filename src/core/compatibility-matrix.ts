...
type CompatKey = `${ArtifactKind}:${TraceEdge}:${"upstream" | "downstream"}`;
...
const MATRIX = new Map<CompatKey, Set<ArtifactKind>>();

function allow(source: ArtifactKind, edge: TraceEdge, dir: "upstream" | "downstream", targets: ArtifactKind[]) {
...
export function isCompatible(
  ...
  const key: CompatKey = `${sourceKind}:${edge}:${direction}`;
