...
### Phase 3.5: RECORD DIFF

After implementation is complete, auto-generate a `generated_diff` artifact that records what changed:

1. Run `git diff --stat` and `git diff --name-only` to collect changed paths.
2. Map each changed path to the behavior specs it implements (from the execution plan's `required_inputs.behaviors`).
3. Write the artifact:

```yaml
kind: generated_diff
schema_version: "0.3"
id: GD-<plan-slug>-<current ISO-8601 timestamp>
title: "Generated diff for <execution plan title>"
status: proposed
owners:
  - <current user>
...
spec:
  ...
  operations:
    - <create_file|modify_file|delete_file for each path>
  rationale:
