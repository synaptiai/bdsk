...
export function validateAcceptance(index, priorFindings) {
    const findings = [];
...
function computeOutcome(index, adId, priorFindings) {
    ...
    const blockingFindings = priorFindings.filter((f) => f.severity === "error" && BLOCKING_CODES.has(f.code) &&
        f.artifact_id !== null && relatedIds.has(f.artifact_id));
    const warningFindings = priorFindings.filter((f) => f.severity === "warning" &&
        f.artifact_id !== null && relatedIds.has(f.artifact_id));
    let outcome;
...
export function computeExecutionResult(index, epId, allFindings) {
    ...
    const verificationArtifacts = index.allOfKind("verification_artifact")
        .filter((v) => v.trace.upstream.some((r) => r.edge === "proves" && index.get(r.target_id)?.kind === "behavior_spec"))
        .map((v) => v.id);
...
//# sourceMappingURL=v8-acceptance.js.map