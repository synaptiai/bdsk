...
// hooks/validate-single-artifact.js
...
var [, , artifactPath, schemasDir] = process.argv;
...
var resolvedSchemas = resolve(schemasDir);
var commonPath = join(resolvedSchemas, "common.json");
var kindSchemaPath = join(resolvedSchemas, `${kind}.json`);
...
var ajv = new import_ajv.default({ allErrors: true, strict: false, validateSchema: false });
import_ajv_formats.default(ajv);
var commonSchema = JSON.parse(readFileSync(commonPath, "utf-8"));
ajv.addSchema(commonSchema, commonSchema.$id ?? "common.json");
var kindSchema = JSON.parse(readFileSync(kindSchemaPath, "utf-8"));
var validate = ajv.compile(kindSchema);
var valid = validate(raw);
if (valid) {
  process.exit(0);
}
var errors2 = validate.errors || [];
